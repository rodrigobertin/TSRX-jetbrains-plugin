...
const jsonTargetFiles = ['src/main/resources/textmate/Syntaxes/tsrx.tmLanguage.json'];

const plistTargetFiles = ['src/main/resources/textmate/info.plist'];
...
async function writeAssetBundleGrammar(sourcePath, targetPath) {
	...
	await import('node:fs/promises').then(m => m.mkdir(path.dirname(targetPath), { recursive: true }));
